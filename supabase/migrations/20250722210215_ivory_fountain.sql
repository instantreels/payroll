...
(3, '2024-07-01', '09:30:00', '18:30:00', 8.00, 'present'),
(3, '2024-07-02', '09:00:00', '18:00:00', 8.00, 'present'),
(3, '2024-07-03', '09:00:00', '13:00:00', 4.00, 'half_day');

-- Create indexes for better performance
CREATE INDEX idx_employees_status ON employees(status);
CREATE INDEX idx_employees_department ON employees(department_id);
CREATE INDEX idx_payroll_period_employee ON payroll_transactions(period_id, employee_id);
...
BEGIN
    ...
    DECLARE comp_id INT;
    DECLARE comp_amount DECIMAL(10,2);
    DECLARE comp_formula TEXT;
    DECLARE calculated_amount DECIMAL(10,2);
    
    DECLARE salary_cursor CURSOR FOR
        SELECT ss.component_id, ss.amount, sc.formula
    ...
    -- Delete existing transactions for this employee and period
    ...
    read_loop: LOOP
        FETCH salary_cursor INTO comp_id, comp_amount, comp_formula;
        IF done THEN
            LEAVE read_loop;
        END IF;
        
        -- Calculate amount (simplified - in real implementation, evaluate formula)
        SET calculated_amount = comp_amount;
        
        -- Insert payroll transaction
        INSERT INTO payroll_transactions (
            employee_id, period_id, component_id, amount, calculated_amount
        ) VALUES (
            emp_id, period_id, comp_id, calculated_amount, calculated_amount
        );
...
DELIMITER //
CREATE FUNCTION GetCurrentFinancialYear() RETURNS VARCHAR(9)
READS SQL DATA
DETERMINISTIC
BEGIN
    ...
    IF current_month >= 4 THEN
        SET fy = CONCAT(current_year, '-', current_year + 1);
    ELSE
        SET fy = CONCAT(current_year - 1, '-', current_year);
    END IF;
...
-- Trigger to update employee count in departments
DELIMITER //
CREATE TRIGGER update_department_count 
AFTER INSERT ON employees
FOR EACH ROW
BEGIN
    -- This could update a department statistics table
    -- For now, just log the change
    INSERT INTO audit_logs (user_id, action, table_name, record_id, new_values)
    VALUES (1, 'employee_added_to_department', 'employees', NEW.id, 
            CONCAT('Department: ', NEW.department_id));
END //
DELIMITER ;

-- Insert initial audit log entry
INSERT INTO audit_logs (user_id, action, table_name, record_id, new_values, ip_address) 
VALUES (1, 'system_initialization', 'system', 1, 'Database initialized with sample data', '127.0.0.1');
